<template>
    <div>
      ...
      <form @submit.prevent="submitForm">
        ...
        <img v-if="form.image_product" :src="'http://localhost:3000/public/img/product/' + form.image_product" alt="Gambar Produk" width="250">
    ...
    methods: {
      async submitForm() {
        try {
          ...
          const response = await axios.put(`http://localhost:3000/api/product/${productId}`, this.form);
      ...
      async uploadImage(image) {
        ...
        const response = await axios.post('http://localhost:3000/api/uploadNewImageProduct', formData, {
          headers: {
